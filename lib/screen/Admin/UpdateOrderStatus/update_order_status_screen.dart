...
class UpdateOrderStatusScreen extends StatefulWidget {
  ...
  const UpdateOrderStatusScreen({Key? key, required this.order})
      : super(key: key);
...
class _UpdateOrderStatusScreenState extends State<UpdateOrderStatusScreen> {
  ...
  void initState() {
    ...
    _loadStatusHistory();
    _loadOrderRelatedData();
  }

  Future<void> _loadOrderRelatedData() async {
  try {
    // Load product
    ...
    if (productSnap.exists && productSnap.data() != null) {
      _product =
          ProductModel.fromMap(productSnap.data() as Map<String, dynamic>);
    } else {
      print('⚠️ Product not found: ${widget.order.productId}');
    }
    ...
    if (userSnap.exists && userSnap.data() != null) {
      _user = UserModel.fromMap(userSnap.data() as Map<String, dynamic>);
    } else {
      print('⚠️ User not found: ${widget.order.userId}');
    }

    // Load user info (optional)
    DocumentSnapshot userInfoSnap = await FirebaseFirestore.instance
        .collection('UserInfo')
        .doc(widget.order.userId)
        .get();

    if (userInfoSnap.exists && userInfoSnap.data() != null) {
      _userInfo =
          UserInfoModel.fromMap(userInfoSnap.data() as Map<String, dynamic>);
    }

    setState(() {}); // Refresh UI if needed
  } catch (e) {
    print('❌ Error loading order-related data: $e');
  }
}
  ...
  Future<void> _selectDeliveryDate() async {
    final DateTime? picked = await showDatePicker(
      context: context,
      initialDate:
          _selectedDeliveryDate ?? DateTime.now().add(Duration(days: 1)),
    ...
    try {
      ...
      if (_messageController.text.trim().isNotEmpty) {
        ...
        DocumentReference statusRef =
            FirebaseFirestore.instance.collection('StatusHistory').doc();
        ...
        if (_selectedOrderStatus == "Approved") {
          if (_user != null && _product != null) {
            print("$_user");
            await _sendApprovalEmail(widget.order);
          } else {
            print('Skipping email: missing user or product data');
          }
        }
      }
  ...
  Future<void> _sendApprovalEmail(OrdersModel order) async {
    const serviceId = 'service_3595h4l';
    const templateId = 'template_zw5hf2x';
    const userId = 'ZeA9uS2LOTiKQyM5W'; // EmailJS public key
    ...
    final response = await http.post(
      ...
      body: json.encode({
        ...
        'template_params': {
          // 'user_email':
          //     _user!.email, // Make sure you have order.email available
          'user_email':
              'owaisaslams911@gmail.com', // Make sure you have order.email available
          'product_name': _product?.name ?? 'Unknown Product',
          'price': order.totalAmount.toStringAsFixed(2),
          'message':
              'We have approved your order. It will be processed shortly.',
